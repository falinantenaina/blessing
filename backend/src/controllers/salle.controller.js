...
export const checkSalleDisponibilite = asyncHandler(async (req, res) => {
  ...
  if (!jour_id || !horaire_id) {
    return errorResponse(res, 'Les paramètres jour_id et horaire_id sont requis', 400);
  }
...
export const getSallesDisponibles = asyncHandler(async (req, res) => {
  ...
  return successResponse(res, salles, 'Salles disponibles récupérées avec succès');
});
...
export const getSalleStats = asyncHandler(async (req, res) => {
  ...
  return successResponse(res, stats, 'Statistiques des salles récupérées avec succès');
});
